...
- **Backend (pytest):** 474 tests green (includes `test_llm_fixture.py`, 22 tests, for the deterministic adapter + fault semantics; the F6 deny-by-default projection suite in `test_portal_projection.py`; a login regression test for the pyjwt≥2.13 empty-secret `InvalidKeyError` caught by the first compose run; F8 adds 11 queue-search tests; F10 adds the assignment/reassignment/filter/workload tests).
