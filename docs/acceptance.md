...
- **Backend (pytest):** 433 tests green (includes `test_llm_fixture.py`, 22 tests, for the deterministic adapter + fault semantics; the F6 deny-by-default projection suite in `test_portal_projection.py`; and a login regression test for the pyjwt≥2.13 empty-secret `InvalidKeyError` caught by the first compose run).
- **Frontend (Vitest/RTL):** 145 tests green in 20 files (`frontend-test` in CI runs the same suite with coverage).
