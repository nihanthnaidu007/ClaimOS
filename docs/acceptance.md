...
- **Frontend (Vitest/RTL):** 180 tests green in 23 files (adds the workbench/portal message-thread suites, 14 tests, with hostile-string XSS assertions) (`frontend-test` in CI runs the same suite with coverage; F8 adds 4 search-state tests; F10 adds the chip and workload-card tests).
