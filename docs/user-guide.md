...
**Send the documents your adjuster asked for.** If your adjuster needs something from you — a repair estimate, a police report — a "Documents we need" list appears on the status page with what to send and any note they left about what makes a good copy. Choose the file (PDF, PNG, or JPEG, up to 10 MB) and upload it against the item: the item flips to "Received" on the spot, and your adjuster is notified. If a file can't be accepted — the wrong type, over the size cap, or an empty file — the page says why in plain words so you can try again. Uploads only work for the claim you're signed in to see, with the same claim number and access code as the rest of the portal.
