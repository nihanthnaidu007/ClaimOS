...
**`POST /api/status/lookup`** — request: `claimNumber` (matches `CLM-YYYYMMDD-N` or `CLM-HIST-N`) and `accessCode` (16–200 chars). → `200` `StatusLookupResponse` — the masked portal payload: `{claimNumber, firstName, status, statusLabel, currentStage, incidentType, decisionOutcome, decisionReady, pdfAvailable, milestones: [{key, label, at, done}], documentRequests: [{id, title, description, status}]}`. Amounts, contact details, and policy numbers never appear. `documentRequests` is deny-by-default: present only when the claim has document requests, and carrying just the id, title, note, and state ("requested" | "received" | "waived") — never internal fields such as `requested_by`, `document_id`, or timestamps.
...
**`POST /api/status/upload-document`** — customer upload answering a document request. Multipart form: `claimNumber`, `accessCode`, `requestId`, `file`. All three credentials must line up — the claim pair must match AND the request must belong to that claim, so a request id from any other claim is the same generic `404` as an unknown one; the request must still be `requested` (`409` once received or waived). The file is validated exactly like adjuster uploads (shared `uploads.py`): allowlisted content type PDF/PNG/JPEG (`415`), 10 MiB cap (`413`), non-empty (`422`); the blob is stored under the claim's key with its SHA-256 hash. On success the uploaded document is linked to the request (`document_id`), the request flips to `received`, the action is audited (`portal_document_uploaded`), a durable `document_request_received` event lands on the claim's stream, and the adjuster who requested the document gets an in-app notification. → `201` `{"requestId", "status": "received", "documentId", "filename", "sizeBytes"}`. Errors: `404` unknown claim/code/request triple; `409` request no longer open; `413`/`415`/`422` file validation. Rate-limited per client IP (`PORTAL_UPLOAD_RATE_LIMIT`, default `10/minute`).
